...
int fs_remove_file(F15FS_t *const fs, const char *const fname){
    ...
    search_struct_t file_search;

    dir_rec_t dir_search;
    inode_t *file_parent_inode = (inode_t *)malloc(sizeof(inode_t));



    locate_file(fs, fname, &file_search);
        if (file_search.valid) {
              inode_t *file_inode = (inode_t *)malloc(sizeof(inode_t));
            dir_block_t *dir_record = (dir_block_t *)malloc(sizeof(dir_block_t));
            if (file_inode && dir_record && load_inode(fs, file_search.inode, file_inode)) {
                if (INODE_IS_TYPE(file_inode, DIRECTORY)) {
                    if(fs_get_dir(fs, fname, &dir_search) == 0){
                        if(dir_search.total != 0){
                            printf("Cannont delete non-empty directories\n");
                            return -1;
                        }
                    ...
                    data_itr_t file_itr;
                    file_itr.inode = file_search.inode;
                    file_itr.location = -1;
                    size_t offset = 0;
                    int blocks = 8;
                   ...
                   while (blocks != 0) {
                        ...
                        if (file_itr.location) {
                            get_position_and_index_tuple(offset, &file_itr);
                            find_block_index(fs, &file_itr);
                            block_store_release(fs->bs, file_itr.location);
                            block_store_release(fs->bs, file_inode->data_ptrs[blocks]);
                            blocks--;
                            continue;
                        }
                    }
                    ...
                    load_inode(fs, file_inode->mdata.parent, file_parent_inode);
                    load_block(fs, file_parent_inode->data_ptrs[0], parent_block);
                   
                   printf("Parent: %s\n", file_parent_inode->fname);
                    printf("Looking for %s\n", file_inode->fname);


                    for (int i = 0; i < DIR_REC_MAX; ++i) {
                         
                        if(strncmp(parent_block->entries[i].fname, file_inode->fname, FNAME_MAX) == 0){
                            printf("Dir Entry %d: %s\n", i, parent_block->entries[i].fname);
                            printf("File compared %s\n", file_inode->fname);
                            memset(parent_block->entries[i].fname, '\0', FNAME_MAX);

                            parent_block->entries[i].inode = 0;
                            memset(file_inode->fname, '\0', FNAME_MAX);
            
                            write_inode(fs, file_search.inode, file_inode);
                            write_block(fs, file_parent_inode->data_ptrs[0], parent_block);
                            return 0;
                        }
                    }
...
int fs_move_file(F15FS_t *const fs, const char *const fname_src, const char *const fname_dst){
    ...
    search_struct_t dir_search, file_src;
    inode_t *file_src_inode = (inode_t *)malloc(sizeof(inode_t));
    locate_file(fs, fname_src, &file_src);
    load_inode(fs, file_src.inode, file_src_inode);


      locate_file(fs, fname_dst, &dir_search);
        if (dir_search.valid) {
            // further analysis, is it actually a directory...?
            // Also, malloc.
            ...
            if (file_inode && dir_record && load_inode(fs, dir_search.inode, file_inode)) {
                if (INODE_IS_TYPE(file_inode, DIRECTORY)) {
                    ...
                    if (load_block(fs, file_inode->data_ptrs[0], dir_record)) {
                        ...
                        for (int i = 0; i < DIR_REC_MAX; ++i) {
                            if (!dir_record->entries[i].fname[0]) {
                               
                                strncpy(dir_record->entries[i].fname, fname_src, FNAME_MAX);
                                dir_record->entries[i].inode = file_src.inode;
                                break;
                            }
                        }
                    }
                    dir_block_t *parent_block = (dir_block_t *)malloc(sizeof(dir_block_t));
                    load_inode(fs, file_src_inode->mdata.parent, file_parent_inode);
                    load_block(fs, file_parent_inode->data_ptrs[0], parent_block);


                    for (int i = 0; i < DIR_REC_MAX; ++i) {
                        if(strncmp(parent_block->entries[i].fname, fname_src, FNAME_MAX) == 0){
                           memset(parent_block->entries[i].fname, '\0', FNAME_MAX);
                            memset(file_inode->fname, '\0', FNAME_MAX);
                            parent_block->entries[i].inode = 0;
                        }
